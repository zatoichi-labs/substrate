...
use consensus_common::{self, Authorities, BlockImport, Environment, Proposer,
	ForkChoiceStrategy, Error as ConsensusError};
	...
	let make_authorship = move || {
		...
		Slots::new(slot_duration, inherent_data_providers)
			...
			.for_each(move |slot_info| {
				...
				let authorities = match authorities::<B, _>(client.as_ref(), &BlockId::Hash(chain_head.hash())) {
...
impl<B: Block, C, E> Authorities<B> for AuraVerifier<C, E> where
	C: ProvideRuntimeApi
{
...
fn authorities<B: Block, C: ProvideRuntimeApi>(
	client: &C,
	at: &BlockId<B>
) -> Result<Vec<AuthorityIdFor<B>>, ConsensusError> {
	client.runtime_api().authorities(at)
}
